...
@app.function_name(name="ProfileEvents")
@app.blob_output(arg_name="OutputToBlob", path="profiles/{dateTime.Now}.json", connection="BlobStorageConnectionString")
@app.service_bus_queue_trigger(arg_name="upsertEvent", queue_name="profilecreatedormodifiedqueue", connection="serviceBusConnectionString")
def checkCreatedOrModified(upsertEvent: func.ServiceBusMessage, OutputToBlob: func.Out[str]):
    logging.info("Python ServiceBus queue trigger processed message: %s", upsertEvent.get_body().decode()) 
    url = upsertEvent.get_body().decode()
...
def _sharepointQuery(access_token, url):
    ...
    #print("file retrieved: ", json.dumps(fileData, indent=2))
